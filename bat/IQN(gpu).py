...
class net(nn.Module):
    ...
    def __init__(self, n_states=4, n_actions=2, quant_num = 64):
        ...
        # for state S to V
        self.convert_to_V = nn.Linear(self.n_states,128) 
    ...
    def forward(self,state):
        ...
        # ϕ for weight and bias
        tau = torch.randn(self.quant_num,1) # τ.size() = [64,1] (選64個τ)
...
class IQN(object):
    ...
    def train(self,ID):
        ...
        print('start'+str(ID))
        EWMA_reward = None
        f1 = open('CartPole-v0_EWMA' + str(ID) + '.txt', 'w+')
        f2 = open('CartPole-v0_score' + str(ID) + '.txt', 'w+')
...
def main(ID):
    ...
    #NN1
    a = IQN(env,episode=400,n=3,n_prime=3,epsilon=0.05,learning_rate=1e-4,gamma=0.95,quant_num=64)
    a.train(ID)
