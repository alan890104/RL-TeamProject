...
class net(nn.Module):
    ...
    '''
    def __init__(self, n_states=3, n_actions=21, quant_num = 64):
        ...
        # for state S to V
        self.convert_to_V = nn.Linear(3,128) 
    ...
    def forward(self,state):
        ...
        H = F.relu(self.phi(tau.cuda())).view(-1,self.quant_num,128)
        # print(H.shape)
        ...
        # print(self.phi(cos_trans).sum(1).shape)
        # H = F.relu(self.phi(cos_trans).sum(1)+self.phi_bias.unsqueeze(0)).unsqueeze(0)# H.size() = [1,64,128]
        V = V.unsqueeze(1)
...
class IQN(object):
    def __init__(self, env=None, capacity=10000, episode=1000, exploration=1000, k_sample=8, k=1, n=3, n_prime=3, gamma=0.9, batch_size=32, epsilon=0.05, learning_rate=1e-3, quant_num=64, update_freq=100, render=True):
        self.env = env
    ...
    def train(self,ID):
        ...
        print('start'+str(ID))
        EWMA_reward = None
        f1 = open('Data/PendulumEnv_EWMA' + str(ID) + '.txt', 'w+')
        f2 = open('Data/PendulumEnv_score' + str(ID) + '.txt', 'w+')
...
def main(ID):
    ...
    a = IQN(env,episode=400,epsilon=0.05,learning_rate=1e-4,gamma=0.95,quant_num=64)
    a.train(ID)
